...
// Add this endpoint to your server.js file to get all masters with their industries
app.get("/api/masters", async (req, res) => {
  try {
    // Get all users with role_master = true and approved status
    const mastersResult = await executeQuery(`
      SELECT u.id, u.username, up.first_name, up.last_name, up.email, 
             up.phone, up.approval_status
      FROM users u
      JOIN user_profile up ON u.id = up.user_id
    ...
    // For each master, get their industries
    for (const master of masters) {
      // Get industries
      const industriesResult = await executeQuery(
        `
    ...
    console.error("❌ Error getting masters list:", err.message);
    res.status(500).json({ message: "Server error", error: err.message });
  }
});

// Optional: Add an endpoint to track call events
app.post("/api/calls", async (req, res) => {
  const { master_id, status } = req.body;
  
  try {
    // You could log this to a calls table in your database
    // For now, we'll just log it to the console
    console.log(`Call ${status} with master ID: ${master_id}`);
